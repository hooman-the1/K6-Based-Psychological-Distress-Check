...
- If the acceptance criteria or project definition of done requires
  automated coverage that is missing, mark the affected criterion FAIL
  and identify the untested behavior
...
Tests: `.\.venv\Scripts\python.exe manage.py test`, [actual passed count] passed,
[actual failed count] failed

Definition of done:

- The comment starts with `## QA: PASS` or `## QA: FAIL`
